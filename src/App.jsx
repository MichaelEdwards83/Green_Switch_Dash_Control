...
import { Activity, Server, AlertCircle, CheckCircle2, X, Zap, Sun, Moon } from 'lucide-react';
          ...
          <div className="modal-content" style={{ maxWidth: '400px' }}>
            <div className="modal-header" style={{ flexDirection: 'column', alignItems: 'flex-start', gap: '0.5rem', position: 'relative' }}>
              <div style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h3 className="port-modal-title">
                  {currentSwitch.name.replace('SW-GRN-', '')} : Port {selectedPort}
                </h3>
                <button className="close-btn" onClick={() => setSelectedPort(null)}><X size={20} /></button>
              </div>

              {currentSwitch.switchDetails?.ports?.[selectedPort]?.description ? (
                <div className="port-description-box">
                  {currentSwitch.switchDetails.ports[selectedPort].description}
                </div>
              ) : (
                <div className="port-description-empty">No Description</div>
              )}
            </div>
            <div className="modal-body">
              <div className="control-group">
                ...
                <div className="input-row">
                  {currentSwitch.switchDetails?.vlanMap && Object.keys(currentSwitch.switchDetails.vlanMap).length > 0 ? (
                    <select className="vlan-select" value={vlanId} onChange={(e) => setVlanId(e.target.value)}>
                      <option value="">Select VLAN...</option>
                      {Object.entries(currentSwitch.switchDetails.vlanMap).map(([id, name]) => (
                        <option key={id} value={id}>
                          {id} - {name}
                        </option>
                      ))}
                      {vlanId && !currentSwitch.switchDetails.vlanMap[vlanId] && <option value={vlanId}>{vlanId} (Custom)</option>}
                    </select>
                  ) : (
              ...
              <div className="control-group">
                ...
                <button className="danger-btn" style={{ width: '100%' }} onClick={cyclePoe}><Zap size={16} /> Reboot Port</button>
              </div>
            </div>
          </div>
        </div>
