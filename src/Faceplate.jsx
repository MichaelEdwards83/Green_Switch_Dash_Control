...
export function SwitchFaceplate({ portCount, portData = {}, systemName, systemModel, vlanMap = {}, onPortClick, connectivity, ipOob, ipTrunk, processingPorts }) {
                ...
                <div className="ports-grid">
                    {portPairs.map((pair, idx) => (
                        <div key={idx} className="port-column">
                            {pair[0] && <Port id={pair[0].id} isTop={true} data={portData[pair[0].id]} color={getVlanColor(portData[pair[0].id]?.vlan)} onClick={() => onPortClick(pair[0].id)} isProcessing={processingPorts?.has(`${props.ipOob}-${pair[0].id}`) || processingPorts?.has(`${props.ipTrunk}-${pair[0].id}`)} />}
                            {pair[1] && <Port id={pair[1].id} isTop={false} data={portData[pair[1].id]} color={getVlanColor(portData[pair[1].id]?.vlan)} onClick={() => onPortClick(pair[1].id)} isProcessing={processingPorts?.has(`${props.ipOob}-${pair[1].id}`) || processingPorts?.has(`${props.ipTrunk}-${pair[1].id}`)} />}
                        </div>
                    ))}
                </div>
            </div>
...
function Port({ id, isTop, data, color, onClick, isProcessing }) {
    const isUp = data?.up || false;
    const hasPoe = data?.poe || false;
    const vlan = data?.vlan;

    let borderColor = color || '#334155';
    let bgColor = color ? `${color}20` : '#0f172a';
    let displayColor = color || '#475569';
    ...
    return (
        <div
            className={`port-box ${isProcessing ? 'processing' : ''}`}
            onClick={onClick}
            title={isProcessing ? `Port ${id} - Updating...` : `Port ${id} - ${vlan ? 'VLAN ' + vlan : 'No Config'}`}
            style={{
                borderColor: borderColor,
                backgroundColor: bgColor,
                boxShadow: (color || isProcessing) ? `0 0 10px ${bgColor}` : 'none',
        ...
        >
            ...
            <div className="vlan-display" style={{ color: displayColor }}>
                {isProcessing ? '...' : (vlan || '-')}
            </div>
